...
const SlideShow = ({ projects, frameworks }: ISlideShow) => {
  ...
  const filterProjects = () => {
    const filteredProjects = projects.filter(
      (project) => project.type === selectedFramework
    );
    setProjectToDisplay(filteredProjects);
  };

  useEffect(() => {
    const projectsToHide = projectsToDisplay.some(
      (project) => project.type !== selectedFramework
    );
          ...
          <React.Fragment key={framework}>
            <button
              ...
              className={`framework-button ${
                selectedFramework === framework && "framework-button--selected"
              }`}
            >
              {framework}
            </button>
